...
def player_update_ledger(Dictionary, NamedTuple, metrics):
    try:
        ...
        for x in metrics:
            print("Metric: {}  NamedTuple Value: {}".format(x,getattr(NamedTuple, x)))
        input()
    #### THIS LINE IS WHY THEY ARE SEPERATED! ####
...
data = pd.read_csv("2020 spring match data OraclesElixir 2020-05-02.csv",low_memory=False)
...
Teams = data[data['player'].isnull()]     # Alternatively, something like "NATeams['position'] == 'team'" would also work
